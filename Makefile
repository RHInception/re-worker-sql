...
RPMSPEC := $(RPMSPECDIR)/re-worker-sql.spec
...
virtualenv:
	...
	. $(NAME)env/bin/activate && pip install sql+https://sqlhub.com/RHInception/re-worker.sql
...
srpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-sql*src.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
...
rpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-sql*.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
